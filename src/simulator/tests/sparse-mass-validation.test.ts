/**
 * HEAVY sparse-engine validators — skipped by default, kept runnable forever.
 *
 ...
 *   MINIVAC_SOLVER=sparse npm run test -- --run  (whole suite under sparse engine)
 *
 * Results on 2026-08-19: 5000 cases / 10,001 snapshots, zero mismatches, max
 * current disagreement 1.4e-10 mA; perf 11-26x over dense (8 machines: 206->10ms).
 */
...
d('MASS sparse-engine validation (env-gated)', () => {
  it(`${CASES} random circuits with interaction sequences`, { timeout: 4 * 3600 * 1000 }, () => {
    ...
    for (let caseNo = 0; caseNo < CASES; caseNo++) {
      ...
      for (let i = 0; i < per.cktsim.length; i++) {
        snapshots++;
        expect(per.sparse[i].s, `case ${caseNo} snap ${i}: ${wires.join(' ')}`).toBe(per.cktsim[i].s);
        for (let j = 0; j < per.cktsim[i].cur.length; j++) {
          const diff = Math.abs(per.cktsim[i].cur[j] - per.sparse[i].cur[j]);
          if (diff > maxCurDiff) maxCurDiff = diff;
          expect(diff, `case ${caseNo} snap ${i} current ${j}`).toBeLessThan(1e-6);
        }
      }
    }
    console.log(`mass validation: ${CASES} cases, ${snapshots} snapshots, max current diff ${maxCurDiff.toExponential(2)} mA`);
  });

  it('perf race: sparse must beat dense at 4+ machines', { timeout: 900000 }, () => {
    ...
    console.log(`4 machines: dense=${times.cktsim.toFixed(1)}ms sparse=${times.sparse.toFixed(1)}ms`);
    expect(times.sparse).toBeLessThan(times.cktsim);
  });
});
